...
public class LikesRepository(AppDbContext context) : ILikesRepository
{
    ...
    public async Task<PaginatedResult<Members>> GetMembersLikes(LikeParams likeParams)
    {
        var query = context.Likes.AsQueryable();
        IQueryable<Members> result;
        ...
        switch (likeParams.Predicate)
        {
            ...
            default: // Mutual likes
                var likeIds = await GetCurrentMemberLikesIds(likeParams.MemberId);
                result = query
                    .Where(x => x.TargetMemberId == likeParams.MemberId && likeIds.Contains(x.SourceMemberId))
