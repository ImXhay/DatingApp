...
public class MessageRepository(AppDbContext context) : IMessageRepository
{
    ...
    public async Task<PaginatedResult<MessageDTO>> GetMessageForMember(MessageParams messageParams)
    {
        ...
        return await paginationHelper.CreateAsync<MessageDTO>(messageQuery, messageParams.PageNumber, messageParams.PageSize);
    }
    ...
    public async Task<IReadOnlyList<MessageDTO>> GetMessagesThread(string currentMemberId, string recipientId)
    {
        await context.Messages
            .Where(x => x.RecipientId == currentMemberId 
                && x.Sender.Id == recipientId && x.DateRead == null)
            .ExecuteUpdateAsync(settler => settler.SetProperty(x => x.DateRead, DateTime.UtcNow));
